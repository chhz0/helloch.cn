<template>
  <div class="github-card" @mouseover="hover = true" @mouseleave="hover = false">
    ...
    <!-- 正常显示 -->
    <a v-else-if="repoData" :href="repoData.html_url" target="_blank" rel="noopener" class="card-content">
      <!-- 左侧信息区 -->
      <div class="repo-info">
        <h3 class="repo-name">
          <!-- <RepositoryIcon class="icon" /> -->
          <svg class="icon" viewBox="0 0 16 16" width="16" height="16">
            <path :d="RepoIcon" :fill="hover ? 'var(--vp-c-text-2)' : 'currentColor'" />
          </svg>
        ...
        <div class="repo-meta">
          <span v-if="repoData.language" class="language">
            ...
            /> -->
            <img class="language" :src="getLanguageIcon(repoData.language)" />
            <!-- {{ repoData.language }} -->
          </span>
          <span class="stars">
            <!-- <StarIcon class="icon" /> -->
...
import { RepoIcon, StarIcon } from "../svg/icons";
...
const getLanguageIcon = (language: string) => {
  ...
  const icons: Record<string, string> = {
    ...
    'TypeScript': url + 'typescript.svg',
    'Python': url + 'python.svg',
    'Java': url + 'java.svg',
    'Vue': url + 'vue.svg',
    'CSS': url + 'css.svg',
    'HTML': url + 'html.svg',
